...
- 虚函数可以根据特定的派生类类型的不同而执行不同代码。这样可以把类型识别的工作交给对象本身去处理。
...
一旦在类的层级关系中加入新的派生类，像这样的代码往往会出现错误。而且，一旦某个派生类的属性改变了，你很难找到并修改所有受影响的代码块。
...
- 使用 `static_cast` 作为值转换时等价于 C 风格转换的类型转换，或某个类指针需要显示的向上转换为基类指针时，或者当你需要显示的转换一个基类指针到派生类指针时。对于后者，你必须保证要转换的对象确实是该派生类的实例。
- 使用 `const_cast` 去掉 `const` 限定符。
- 使用 `reinterpret_cast` 进行指针类型和其他指针类型或整型之间不安全的相互转换。仅在你对所做一切了然于心时使用。
...
仅在流是解决当前问题的最好工具时才使用流。典型的使用场景是当 I/O 是格式化的、本地的、对人类阅读友好的，并且以其他开发者为目标而不是终端用户时。与和你相关的代码保持统一，并且和整个代码仓库保持统计；当已经存在一个稳定的工具可以解决你的问题时，直接使用该工具作为替代。尤其是使用日志库输出调试信息相对使用 `std::cerr` 或者 `std::clog` 通常是一个更好的选择。

不要对外部用户或者不信任的数据使用流 I/O。而应该找到并使用一些合适的库来处理类似于国际化、本地化和安全性的问题。

如果你一定要使用流，不要使用流 API 中关于状态的部分 （除了错误状态），比如 `imbue()`、`xalloc()` 和 `register_callback()`。使用显示的格式化函数（详见 [fmt](https://fmt.dev/latest/index.html)）而不是流运算符或者格式化标志来控制格式化的细节，比如数字的进制，精度或者填充。
...
- 对于某些整数类型，正确的可移植 printf()转换描述符所依赖的宏扩展（来自 <cinttypes> 的 PRI 宏）非常难用。除非针对特殊情况没有更合理的选择，否则请尝试避免使用甚至升级依赖于 printf 系列的 API。更好的方式是使用类型安全的整数格式库，例如 [fmt](https://fmt.dev/latest/index.html) 进行快速简单转换，或使用 `std::ostream`。
...
对于指针（地址值），到底是用 `0`，`nullptr` 还是 `NULL`：
...
for (Foo& foo : container_of_foos) {
  foo.DoSomethingMutable();
}
```

数组和 STL 中的容器都支持这种遍历方式。实际上，一个自定义容器类型 X，只要支持返回迭代器的 `X::begin()/X::end()` 成员函数或者 `begin(X&)/end(X&)` 自由函数，那么编译器就支持使用基于范围的 for 循环来对 X 进行遍历。

**优点**

- 使用基于范围的 for 循环能够使得代码更简单，语义更直观。

**结论**

在条件允许的情况下，推荐优先选择基于范围的 for 循环来遍历容器。
...
除非case语句处理了所有可能的值，否则 `switch` 应该总是包含一个 `default`匹配。如果`default` 永远执行不到，简单的使用 `assert`语句：
...
当需要对所有枚举值进行处理时，可以不添加 `default` 匹配。原因是，通过设置相关的编译选项，可以在编译阶段发现未处理的枚举值。例如，下面的代码将编译不通过：

```cpp
// GCC: [-Wswitch] 已隐含在 -Wall 中。
// MSVC: C4062 为 4 级警告，需要开启。
enum Abc {
  ABC_A = 0,
  ABC_B = 1,
};

std::string ToString(Abc abc) {
  switch (abc) {
    case ABC_A:
      return "ABC_A";
  }
...
- 所有基类名应在\<line-length\>限制下，尽量与派生类名放在同一行。
...
C++11 之后允许在类中对成员变量进行就地初始化(default member initializer)，建议在条件允许的情况下优先使用这种初始化方式，它有以下优点：
- 代码更简洁直观
- 可以避免不同版本构造函数之间的代码重复及潜在的不一致等问题
...
**基本原则是：** 一屏显示的代码越多，程序的控制流越容易理解。当然，过于密集的代码块和过于疏松的代码块同样难看，这取决于你的判断，但通常是空行越少越好。
...
- 一般情况注释前增加一行空行，用于分隔可读块。不同用途语句块（例如变量定义、函数调用、返回等）之间适当增加空行，可以快速让读者了解整段代码含义。

  ```cpp
  // 好（代码摘自 google protobuf: text_format.cc）
  bool ConsumeDouble(double* value) {
    bool negative = false;

    if (TryConsume("-")) {  /* 在函数体块变量声明和执行语句间增加空行 */
      negative = true;
    }

    // A double can actually be an integer, according to the tokenizer.  /* 注释前空行 */
    // Therefore, we must check both cases here.
    if (LookingAtType(io::Tokenizer::TYPE_INTEGER)) {
      // We have found an integer value for the double.  /* 块起始行的注释不要空行 */
    ...
    if (negative) { /* 上一个 if 和这一个 if 的职能间不同，通过空行增加可读性 */
      *value = -*value;
    }
  ...
  // 不好
  void SomeReport() {
    auto& file_descriptor = application_.file_descriptor();
    auto& application_meta = application_.application_meta();
    // 模块维度元数据上找  /* 注释前最好空行 */
    auto module_logger = make_shared<ModuleReporter>();
    module_logger->set_server_name(file_descriptor.package().c_str());
    module_logger->set_server_component_number(component_.size());
    module_logger->set_server_component_type_number(application_meta.size());
    module_logger->set_server_infrastructure(server_infrastructure_.c_str());
    reports_.emplace_front(move(module_logger));  /* 上面大段代码是赋值，这个操作是压入上报器，可选增加空行 */
  }
  ```

- 在多重 `if-else` 块和 `switch-case` 块中适当增加空行可以增强阅读性。

  ```cpp
  // 好（摘自 google protobuf: descriptor.cc)
  bool ExamineIfOptionIsSet(/* ... */) {
    ...
    for (int i = 0; i < unknown_fields.field_count(); i++) {  /* 大段语句块之间增加空行提升易读性 */
      if (unknown_fields.field(i).number() ==
        ...
        switch (type) {
          case FieldDescriptor::TYPE_MESSAGE:
            ...
            break;  /* 当 switch 中的语句比较长（例如 > 4）时增加空行提升可读性 */
          ...
          default:  /* default 语句前可增加空行以区分不同的 case label */
...
- 大段相同操作的块之间可以适当空行，增加代码可读性。
  ...
  int SomeModule::SomeBusinessProcess() {
    ...
    req.set_uid(base_request_.uin());
    req.set_pub_key(sm2_pub_key_);
    req.set_pub_key_type(mycrypoto::keytype::EncryptPubKeyType_SM2);
    req.set_device_id(actual_request_.device_id());
    req.set_security_level(security_level_);
...
为保持一致性，UE 游戏引擎相关的代码风格采用 [腾讯 UE C++ 代码规范](UE.md)。
