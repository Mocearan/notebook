...
​		`union`是一个结构体，所有的成员都是对同一地址上分配的内存的不同转译。`union`的大小是最大成员的大小。
...
​		The use of “naked” **union**s is best minimized.

​		标准库类型variant可用于消除联合的大多数直接使用。变量存储一组可选类型之一的值。
