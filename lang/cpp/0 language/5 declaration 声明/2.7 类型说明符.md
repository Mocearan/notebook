...
​		C++ has two categories of compound types that allow for this: the enumerated types (including unscoped and scoped enumerations), and the class types (including structs, classes, and unions).
...
​		To access a specific member variable, we use the **member selection operator** (`operator.` and `operator->`) in between the struct variable name and the member name.
