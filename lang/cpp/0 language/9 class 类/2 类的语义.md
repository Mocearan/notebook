...
>
> ​		移动语义有点特别，只有在类除了显式值语义之外，没有其他语义的显式实现时，才会提供默认的移动语义支持。
...
​		使用移动语义时有个问题，就是如何在`l-value`调用移动语义。`l-value`的行为是拷贝语义的。
...
​		当一个左值对象通过`std::move`将资源的所有权转移到目标对象，在所有权转移发生后，移动操作被异常终端，那么源左值对象的被修改状态无法恢复.
