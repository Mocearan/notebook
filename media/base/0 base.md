...
- 音频时间戳：`pts = inc++ * (frame_size * 1000 / sample_rate)`
  ...
  - 对采样率为44.1kHz的AAC音频进行解码时，一帧的解码时间须控制在23.22毫秒内
    ...
    - 则每秒采样数据大致可以封装为`44100 / 1024 = 43`个AAC帧
    - 则每帧的播放时间大致为`1000ms / 43 = 23.22 ms`
...
>
> - 使用两块大小固定的`buf`存放`pcm`
>   - 大小固定，则播放一个`buf`的时间就是固定的，假设`30ms`
> - 两个`buf`的交替播放
> - 播放完一个`buf`进行交换时，说明过了`30ms`
>   - 有可能实际过了`40ms`，但实际数据的播放时间是`30ms`，通过回调通知一次`30ms`到达
> - 再去用视频的播放时间去匹配音频回调到的时间，这时候的时间就是准确的
> - 这里可能存在同步差值，但人眼对视频的视觉暂留不能分辨这些细微的差值
>
...
>
> - 两个`buf`，一个播放一个写数据
> - 写数据的线程在写满后不返回，等到播放的`buf`返回，就是`30ms`
> - 此时返回并交换，然后此时进行视频到音频的同步
