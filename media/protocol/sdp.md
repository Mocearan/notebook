...
- SDP 协议文档 [RFC 2327](https://link.juejin.cn/?target=https%3A%2F%2Ftools.ietf.org%2Fhtml%2Frfc2327)。

---

​		SDP用于描述媒体详情和媒体协商的协议。是一种简短的结构化文本描述，传达了会话的名称和目的、媒体、协议、编解码器格式、时间和传输信息，为会话通知、会话初始和其它形式的多媒体会话初始等操作提供服务。

​		SDP 的设计宗旨是通用性协议，应用范围广泛，但不支持会话内容或媒体编码的协商操作。 所以在流媒体中只用来描述媒体信息。
...
​		如果你需要了解 SDP 数据格式的详细信息，可以在 RFC 8866 中找到。



## 协议格式

​		sdp 格式由多行的`＜type＞=<value>[CRLF]`组成，SDP 信息是文本信息，UTF-8 编码采用 ISO 10646 字符设置。
...
​		该URL的取值有两种：简捷方式和普通方式。

- 简捷方式直接采用产生该历史媒体的媒体源（如某个摄像头）的设备ID以及相关参数，参数用“：”分隔；
- 普通方式采用`http://储存设备ID[/文件夹]*/文件名`；
...
- 其中第一位为历史或者实时媒体流的标识位，0为实时，1为历史；
- 第2位到第6位取20位SIP监控域ID之中的4-8位作为域标识；
- 第7-10位作为域内媒体流标识，是一个与当前域内产生的媒体流SSRC值后4位不充分的四位十进制整数；
...
​		用于文件下载时控制下载速度。指下载倍数（整数）。

#### `a=filesize`

​		文件大小，用于文件下载时的进度计算。单位`bytes`。

#### `a=setup`

​		 a=setup：TCP连接方式（表示本SDP发送者在建立RTP over TCP连接时是主动还是被动发起TCP连接，“active”为主动，“passive”为被动）

#### `a=svcspace`

​		SVC参数，用于视频传输时的分辨率或者帧频控制。

​		空域编码方式 【取值整型。 0：不使用  1：1级增强  2：2级增强  3：3级增强 】  
