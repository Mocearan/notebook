...
​		数据准备：根据I/O操作的就绪状态，分为阻塞I/O和非阻塞I/O。

> 其实质是，请求方发起的请求不会被立即响应，请求方无法放弃操作持续等待称为阻塞；请求方可立即放弃操作决定下一步操作称为非阻塞。

​		数据读写：根据应用进程和内核的交互方式，分为同步I/O和异步I/O。

> 其实质是，请求方主动拉取数据，称为同步；请求方被动接收应答方推送数据称为异步。

​		一般在处理I/O时，阻塞和非阻塞都是同步I/O，只有在使用特殊系统I/O才能进行真正的异步I/O。
...
​		操作系统内核协议栈，会为已连接的socket fd配套基础设施：发送缓冲区。缓冲区大小可以通过套接字选项来改变。

>   ​		不能无限制增大缓冲区，内核缓冲区总是充满数据时会产生粘包问题，同时网络的传输大小MTU也会限制每次发送的大小进行分片发送，由于数据阻塞需要消耗大量内存资源，呢资源使用效率也得不到提高。
>
>   ​		udp没有发送缓冲区，只有已连接的tcp socket fd才会有发送缓冲区。

​		发送函数实际上不是将数据直接通过网络设备发送，而是拷贝到内核协议栈的发送缓冲区中，等待合适的时机进行发送。

​		阻塞发送函数的返回是，将所有数据完全拷贝至发送缓冲区后返回：
